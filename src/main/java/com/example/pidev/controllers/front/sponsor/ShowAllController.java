package com.example.pidev.controllers.front.sponsor;
...
public class ShowAllController implements Initializable {
    ...
    public Parent makeSponsorModel(
        ...
        try {
            ...
            ((Button) innerContainer.lookup("#editButton")).setOnAction((ignored) -> modifierSponsor(sponsor));
    ...
    private void ajouterSponsor(ActionEvent ignored) {
        currentSponsor = null;
        MainWindowController.getInstance().loadInterface(Constants.FXML_FRONT_MANAGE_SPONSOR);
    }
        ...
        if (action.isPresent()) {
            if (action.get() == ButtonType.OK) {
                if (SponsorService.getInstance().delete(sponsor.getId())) {
                    MainWindowController.getInstance().loadInterface(Constants.FXML_FRONT_DISPLAY_ALL_SPONSOR);
                } else {
